...
/**
 * this manager is in charge of the Guide Messages only, on its method it check if the message is good to go and send it
 * to the right fifoManager
 * 
 */
public class GuideManager extends MessageManager {
	...
	private boolean isForMe(Message message) {
		...
		if (guided.getUserDestination().equals(this.getUser())) {
			return true;
		} else {
			return false;
		}
